...
const AdminInterCityByCityReport = () => {
  ...
  const fetchInterCity = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/admin/reports/intercity-by-city?period=${period}`);
      setData(res.data);
    } catch (err) {
    ...
    <Container>
      <Box display="flex" justifyContent="space-between" alignItems="center" my={3}>
        <Typography variant="h5" fontWeight="bold">
          Transferts Interville par Ville de retrait ({period})
        </Typography>
...
export default AdminInterCityByCityReport;
