...
const AdminCommissionsTaxesReport = () => {
  ...
  const fetchCommissions = async () => {
    setLoading(true);
    try {
      const res = await api.get(`/admin/reports/commissions-taxes?period=${period}`);
      setData(res.data);
    } catch (err) {
    ...
    <Container>
      <Box display="flex" justifyContent="space-between" alignItems="center" my={3}>
        <Typography variant="h5" fontWeight="bold">
          Commissions & Taxes par Ville ({period})
        </Typography>
...
export default AdminCommissionsTaxesReport;
